const getEmployees = async () => {
  try {
    ...
    for (const employee of data) {
      ...
      const attendanceDays = await calculateAttendance(
        employee.ID,
        selectedMonth
      );
...
const calculateAttendance = async (employeeId, selectedMonth) => {
  try {
    ...
    const res = await fetch(
      `http://localhost:3000/attendance?employeeId=${employeeId}&month=${selectedMonth}`,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
    ...
    for (const record of attendanceData) {
      if (employeeId.toString() === record.ID_NV) {
        // Lấy ngày từ cột 'DATE' của dữ liệu chấm công
        const date = new Date(record.DATE);
        totalWorkingDays = record.AttendanceCount;
